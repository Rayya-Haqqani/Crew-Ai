...
class MeetingPrepTasks():
    def research_task(self, agent, meeting_participants, meeting_context):
        return Task(
            ...
            async_execution=True, # async meaning two tasks can be run at the same time
            agent=agent
        )

    def industry_analyst_task(self, agent, meeting_participants, meeting_context):
        return Task(
            description=dedent(f"""\
              Analyze the current industry trends, challenges, and opportunities relevant 
              to the meeting context. Consider market reports, recent developments, and expert 
              opinions to provide a comprehensive overview of the industry landscape.
            ...
            async_execution=True,  # async = True meaning two tasks can be run at the same time
            agent=agent
        )
    ...
    def meeting_strategy_task(self, agent, meeting_context, meeting_objective):
        return Task(
            ...
            expected_output=dedent("""\
              ...
              to help achieve the meeting objective during the meeting."""),
            agent=agent
        )
